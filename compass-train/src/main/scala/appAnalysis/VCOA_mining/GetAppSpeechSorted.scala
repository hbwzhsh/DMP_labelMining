...
object GetAppSpeechSorted {
  def main(args: Array[String]) {
    ...
    val infile1 = ShareSparkMethod
      ...
      ).groupByKey().map(x=>(x._1,x._2.toList.sortBy(_._1).map(x=>x._2)))
      .map(x=>x._1+"\t"+x._2.mkString("~"))
      .repartition(1)


    infile1.saveAsTextFile("/user/lxbai/project5/inputContentGrouped/"+args(0)+"/")
